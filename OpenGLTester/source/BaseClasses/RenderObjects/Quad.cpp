...
void Quad::makeQuad(glm::uvec2 _size, glm::vec3 _position, unsigned int _textureID)
{
    ...
    v2->Position = vec4(0.f, 0.f, 0.f, 0.f) + m_position + vec4(-(_size.x/2.f), (_size.y/2.f), 0.f, 0.f);
    v2->UV = vec2(0.f, 1.f);
    ...
    v3->Position = vec4(0.f, 0.f, 0.f, 0.f) + m_position + vec4((_size.x/2.f), (_size.y/2.f), 0.f, 0.f);
    v3->UV = vec2(1.f, 1.f);
    ...
    v4->Position = vec4(0.f, 0.f, 0.f, 0.f) + m_position + vec4((_size.x/2.f), -(_size.y/2.f), 0.f, 0.f);
    v4->UV = vec2(1.f, 0.f);
...
void Quad::setPosition(glm::vec3& _newPosition)
{
    ...
    {
        vertex->Position =
            vec4(m_size.x * vertex->UV.x, m_size.y * vertex->UV.y, 0.f, 0.f) +
            getOffsetFromCenter(i) +
            vec4(_newPosition, 0.f);
        i++;
    }
}
...
vec4 Quad::getOffsetFromCenter(const unsigned& index)
{
    switch (index)
    {
    case (0):
        return vec4(-(m_size.x/2.f), -(m_size.y/2.f), 0.f, 0.f);
    case(1):
        return vec4(-(m_size.x/2.f),  (m_size.y/2.f), 0.f, 0.f);
    case(2):
        return vec4( (m_size.x/2.f),  (m_size.y/2.f), 0.f, 0.f);
